...
class TestJobSubmissionAPI:
    ...
    def test_submit_job_success(self):
        resp = self.client.post(reverse('submit-job'), {"prompt": "Hello world"}, format='json')
    ...
    def test_cost_saved_on_job(self):
    ...
    # --- Job List ---
    def test_job_list_returns_user_jobs(self):
    ...
    def test_job_list_excludes_other_users(self):
        self.client.post(reverse('submit-job'), {"prompt": "Mine"}, format='json')
        # Switch to another user
        other = User.objects.create_user(username='other', password='OtherPass1!', wallet_balance=Decimal('10.00'))
